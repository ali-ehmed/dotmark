...
class TimeSlot < ActiveRecord::Base
    ...
    Rails.cache.fetch("non_fridays", expires_in: 1.hour) do
		  includes(:time_tables).select("start_time, end_time").where.not(week_day: "Friday").group("start_time, end_time").order("start_time")
    end
	}

	scope :fridays, -> {
    Rails.cache.fetch("fridays", expires_in: 1.hour) do
      includes(:time_tables).select("start_time, end_time").where(week_day: "Friday").group("start_time, end_time").order("start_time")
    end
  }

	scope :week_days, -> {
    Rails.cache.fetch("week_days", expires_in: 1.hour) do
      includes(:time_tables).select("week_day").group("week_day").order("CASE week_day
                                                                            WHEN 'Monday'
                                                                            THEN 1
                                                                            WHEN 'Tuesday'
                                                                            THEN 2
    ...
    Rails.cache.fetch("week_days_#{week_day}", expires_in: 1.hour) do
      includes(:time_tables).where(week_day: week_day)
    end
  }
